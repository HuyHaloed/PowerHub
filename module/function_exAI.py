...
def build_system_message():
    """
    Hệ thống sẽ mô tả nhiệm vụ NLU và schema JSON (Không được hỗ trợ với Openrouter nhưng vẫn hoạt động tốt).
    """
...
If the user asks "brightness", the action is "query_brightness", and the device is "light_sensor".
...
def control_device(device, action, user_text=None):
    ...
    elif device == "light_sensor" and action == "query_brightness":
        brightness = get_brightness()
        return f"Brightness now is {brightness} lux." if brightness else "Unable to get brightness data."
